...
Level.prototype.obstacleAt = function(pos,size){
	//Logic for bounding box
	var xStart = Math.floor(pos.x);
	var xEnd = Math.ceil(pos.x + size.x);
	var yStart = Math.floor(pos.y);
	var yEnd = Math.ceil(pos.y + size.y);
	
	//Boundaries of entire level
...
var wobbleSpeedThing = 30, wobbleDistThing = .04;
Thing.prototype.act = function(step) {
  this.wobble += step * wobbleSpeedThing;
  var wobblePos = Math.sin(this.wobble) * wobbleDistThing;
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if(obstacle){
	//check if lava, and respawn player if yes
	if(obstacle == "lava"){
